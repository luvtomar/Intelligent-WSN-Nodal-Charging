...
global EV N periods home_vehicles Pt_available_EU Ct_available_EU pc home_vehicle_SOC_predictions_param
...
for t = 1:periods
    ...
    for i = 1:N
        ...
        if func == 1 %if the vehicle is not at the charging station
            ...
            if ismember(i,home_vehicles) && ((t >= EV(i).schedule(2) && t < EV(i).schedule(3)) || (t >= EV(i).schedule(4) && t < EV(i).schedule(5)))%if the vehicle is in transit
                ...
                if t == EV(i).schedule(4) %when the vehicle leaves home, record the current state of charge
                    ...
                    home_vehicle_SOC_predictions_param(2,index) = EV(i).soc(t);
                end
            elseif ismember(i,home_vehicles) && (t >= EV(i).schedule(3) && t < EV(i).schedule(4)) && EV(i).soc(t - 1) > EV(i).mr %if the vehicle is at home discharging
                ...
                if t == EV(i).schedule(3) %when the EV reaches home, record the current state of charge
                    [index, a] = ismember(i,home_vehicles);
                    home_vehicle_SOC_predictions_param(1,index) = EV(i).soc(t);
                end
        ...
        %If the constraints on switching numbers are not met, x, y and soc
        %values are assigned the same value as the previous period.
        %Resulting modes and switching are re-calculated for the current
        %time period.
        if equation_18_constraint(i) == 0
            try
                EV(i).soc(t) = EV(i).soc(t-1);
                EV(i).x(t) = EV(i).x(t - 1);
                EV(i).y(t) = EV(i).y(t - 1);
        ...
        %Update current number of unfulfilled
        EV(i).z(t) = EV(i).u - EV(i).isoc - sum(EV(i).x) + sum(EV(i).y);
        if EV(i).z(t) < 0
            EV(i).z(t) = 0;
        end
        %Update currrent profit/cost
        EV(i).profit = EV(i).profit + EV(i).y(t)*Pt_available_EU(t) - EV(i).x(t)*Ct_available_EU(t) - pc*EV(i).z(t) - pc*EV(i).sw(t);
